 ...
 *
 *   $Id: login.php,v 1.1 2004/02/10 15:39:41 bjorn Exp $
 *
...
$board_config['server_name'] = 'localhost/~bjorn/RPGDX';

// session id check
if (!empty($HTTP_POST_VARS['sid']) || !empty($HTTP_GET_VARS['sid']))
{
	$sid = (!empty($HTTP_POST_VARS['sid'])) ?
        $HTTP_POST_VARS['sid'] :
        $HTTP_GET_VARS['sid'];
}
else
    ...
    isset($HTTP_POST_VARS['logout']) || isset($HTTP_GET_VARS['logout']) )
{
        ...
        !$userdata['session_logged_in'])
	{
		...
		else
		{
			...
			$message =
                $lang['Error_login'] . '<br /><br />' .
                sprintf($lang['Click_return_login'],
                    "<a href=\"login.$phpEx?redirect=$redirect\">", '</a>') .
                '<br /><br />' . 
                sprintf($lang['Click_return_index'],
        ...
        isset($HTTP_POST_VARS['logout'])) && $userdata['session_logged_in'])
	{
		...
		{
			$url = (!empty($HTTP_POST_VARS['redirect'])) ?
                $HTTP_POST_VARS['redirect'] : $HTTP_GET_VARS['redirect'];
			redirect(append_sid($url, true));
		}
		else
		{
...
else
{
	...
	if( !$userdata['session_logged_in'] )
	{
		...
		if (isset($HTTP_POST_VARS['redirect']) ||
            isset($HTTP_GET_VARS['redirect']) )
		{
			$forward_to = $HTTP_SERVER_VARS['QUERY_STRING'];

			if( preg_match("/^redirect=([a-z0-9\.#\/\?&=\+\-_]+)/si",
                    $forward_to, $forward_matches) )
			{
				$forward_to = ( !empty($forward_matches[3]) ) ?
                    $forward_matches[3] : $forward_matches[1];
		...
		$username = ( $userdata['user_id'] != ANONYMOUS ) ?
            $userdata['username'] : '';
