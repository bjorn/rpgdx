...
else if ($page == 'stats')
{
	...
	$row = mysql_fetch_object(doQuery("SELECT username, user_posts FROM ". USERS_TABLE ." WHERE username != 'Anonymous' ORDER BY user_posts DESC LIMIT 1"));
	$stats->highest_postcount_username = $row->username;
	$stats->highest_postcount = $row->user_posts;
	$row = mysql_fetch_object(doQuery("SELECT COUNT(project_id) AS project_count FROM ". PROJECTS_TABLE ." WHERE progress_id = 6"));
	$stats->finished_project_count = $row->project_count;
	$row = mysql_fetch_object(doQuery("SELECT AVG(review_score) AS average_review_score FROM ". REVIEWS_TABLE));
	...
	$template->assign_vars(array(
		...
		'ACTIVE_MEMBERS_PERC'        => round(($activeusercount / $usercount) * 100),
		...
		'FINISHED_PROJECTS_PERC'     => round(($stats->finished_project_count / $projectcount) * 100),
		'REVIEW_COUNT'               => $reviewcount,
		'AVERAGE_REVIEW_SCORE'       => sprintf("%01.2f", $stats->average_review_score),
