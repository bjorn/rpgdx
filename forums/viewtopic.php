...
if ( isset($_GET['view']) && empty($_GET[POST_POST_URL]) )
{
	if ( $_GET['view'] == 'newest' )
	{
		...
		//
		$userdata = session_pagestart($user_ip, PAGE_REDIRECT);
		...
		else if ( isset($_COOKIE[$board_config['cookie_name'] . '_sid']) || isset($_GET['sid']) )
		{
			$session_id = isset($_COOKIE[$board_config['cookie_name'] . '_sid']) ? $_COOKIE[$board_config['cookie_name'] . '_sid'] : $_GET['sid'];
...
$page_subtitles = array(
	array($forum_name, $view_forum_url),
	array($topic_title, append_sid("viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;start=$start&amp;postdays=$post_days&amp;postorder=$post_order&amp;highlight=$highlight"))
);
...
for($i = 0; $i < $total_posts; $i++)
{
	...
	//
	if ( $userdata['session_logged_in'] && $postrow[$i]['post_time'] >= $topic_last_read )
	{
	...
	$quote_img = '[<a href="' . $temp_url . '" title="'. $lang['Reply_with_quote']. '">quote</a>]';
	...
	if ( ( $userdata['user_id'] == $poster_id && $is_auth['auth_edit'] ) || $is_auth['auth_mod'] )
	{
		...
		$edit_img = '[<a href="' . $temp_url . '" title="'. $lang['Edit_delete_post']. '">edit</a>]';
	...
	if ( $is_auth['auth_mod'] )
	{
		...
		$ip_img = '[<a href="' . $temp_url . '" title="'. $lang['View_IP']. '">ip</a>]';
		$ip = '<a href="' . $temp_url . '">' . $lang['View_IP'] . '</a>';

		$temp_url = "posting.$phpEx?mode=delete&amp;" . POST_POST_URL . "=" . $postrow[$i]['post_id'] . "&amp;sid=" . $userdata['session_id'];
		$delpost_img = '[<a href="' . $temp_url . '" title="'. $lang['Delete_post']. '">x</a>]';
