...
if ( $mark_read == 'topics' )
{
	...
	$sql =
		...
		"WHERE t.forum_id = $forum_id ".
			"AND p.post_time >= ". $userdata['user_lastread'];

	if ( !($result = $db->sql_query($sql)) ) {
		message_die(GENERAL_ERROR, 'Could not obtain last post times', '', __LINE__, __FILE__, $sql);
	}

	while ($row = $db->sql_fetchrow($result)) {
		$sql = "DELETE FROM ". READS_TABLE ." WHERE read_topic_id = ". $row['topic_id'] ." AND read_user_id = ". $userdata['user_id'];
		if ( !($db->sql_query($sql)) ) {
		...
		$sql =
			...
			"(". $row['topic_id'] .", ". $userdata['user_id'] .", ". time() .")";
		if ( !($db->sql_query($sql)) ) {
...
while( $row = $db->sql_fetchrow($result) )
{
	$moderators[] = '<a href="' . viewprofile_url($row['user_id']) . '">' . $row['username'] . '</a>';
}
...
{
	$topic_days = intval(( !empty($HTTP_POST_VARS['topicdays']) ) ? $HTTP_POST_VARS['topicdays'] : $HTTP_GET_VARS['topicdays']);
...
$page_subtitles = array(
	array($forum_row['forum_name'], append_sid("viewforum.$phpEx?" . POST_FORUM_URL ."=$forum_id"))
);
...
if( $total_topics )
{
	for($i = 0; $i < $total_topics; $i++)
	{
		...
		else
		{
			...
			if( $userdata['session_logged_in'] )
			{
				if( $topic_rowset[$i]['post_time'] >= $userdata['user_lastread'] &&
					  $topic_rowset[$i]['post_time'] >= $topic_rowset[$i]['read_time']) 
				{
		...
		$topic_author = ( $topic_rowset[$i]['user_id'] != ANONYMOUS ) ? '<a href="' . viewprofile_url($topic_rowset[$i]['user_id']) . '">' : '';
		...
		$last_post_author = ( $topic_rowset[$i]['id2'] == ANONYMOUS ) ? ( ($topic_rowset[$i]['post_username2'] != '' ) ? $topic_rowset[$i]['post_username2'] . ' ' : $lang['Guest'] . ' ' ) : '<a href="' . viewprofile_url($topic_rowset[$i]['id2']) . '">' . $topic_rowset[$i]['user2'] . '</a>';
