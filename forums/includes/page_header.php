...
$template->assign_vars(array(
	...
	'PRIVATE_MESSAGE_INFO' => strtolower($l_privmsgs_text),
	...
	'L_LOGIN_LOGOUT' => strtolower($l_login_logout),
	'L_LOGIN' => $lang['Login'],
	'L_LOG_ME_IN' => $lang['Log_me_in'],
	'L_AUTO_LOGIN' => $lang['Log_me_in'],
	'L_INDEX' => sprintf($lang['Forum_Index'], $board_config['sitename']),
	'L_REGISTER' => strtolower($lang['Register']),
	'L_PROFILE' => strtolower($lang['Profile']),
	'L_SEARCH' => strtolower($lang['Search']),
	'L_PRIVATEMSGS' => strtolower($lang['Private_Messages']),
	'L_WHO_IS_ONLINE' => $lang['Who_is_Online'],
	'L_MEMBERLIST' => 'members' /*strtolower($lang['Memberlist'])*/,
	'L_FAQ' => strtolower($lang['FAQ']),
	'L_USERGROUPS' => 'groups' /*strtolower($lang['Usergroups'])*/,
...
else
{
	...
	// Additional _common_ variables when user is logged in
	$template->assign_vars(array(
		'USERNAME' => $userdata['username'],
		'USER_LASTREAD' => $userdata['user_lastread'])
	);
...
/*
 BEGIN OF RPGDX HACKERY!!
*/
/*
	// Add the different RPG types and article categories to the menu.

	// Get counts from different genres
	$result = $db->sql_query("SELECT project_type, COUNT(*) AS number FROM ". PROJECTS_TABLE ." GROUP BY project_type");
	while ($row = mysql_fetch_object($result)) {
		$rpg_genre[$row->project_type] = $row->number;
	}

	// Add each RPG type
	$result = $db->sql_query("SELECT * FROM ". PROJECT_TYPES_TABLE);
	while ($row = mysql_fetch_object($result))
	{
      ...
      'NAME'  => strtolower($row->type_title),
			'URL'   => append_sid('http://www.rpgdx.net/showcategory.php?cat='. $row->type_id),
			'COUNT' => $rpg_genre[$row->type_id])
		);
	}

	// Get counts from different article categories
	$result = $db->sql_query("SELECT article_type, COUNT(*) AS number FROM ". ARTICLES_TABLE ." GROUP BY article_type");
	while ($row = mysql_fetch_object($result)) {
		$rpg_category[$row->article_type] = $row->number;
	}

	// Add each article category
	$result = $db->sql_query("SELECT * FROM ". ARTICLE_TYPES_TABLE);
	while ($row = mysql_fetch_object($result))
	{
      ...
      'NAME'  => strtolower($row->type_title),
			'URL'   => append_sid('http://www.rpgdx.net/showarticles.php?cat='. $row->type_id),
